...
#v1.3
#RS:Added support for complex events
######################
...
my $now = time;
my (
	$filteredMutationVcfFile, $bamFile, $deleteFiles,
	$QSUB,                    $BSUB,    $refFile,
	$mbq,                     $mmq,     $pathToSamtools,
	$outdir,                  $outFile, $pathToBedtools,
	$mpileUpOutFile,          $bamId,   $queue,
	$typeOfSample,            $process_name,
	$w_dir
);
if (
	@ARGV < 3
...
if( $w_dir )	{
	$logger->start_local( $logger, $w_dir );
}
else	{
	$logger->start_local( );
}
$process_name = ( $process_name ? "${process_name}::GTA" : "GTA" );

if ( ( !defined $filteredMutationVcfFile ) or ( !defined $bamFile ) ) {
	$logger->fatal(
"$process_name: Mutation file or .bam file is missing. Please enter filtered Mutation file in as vcf format. Also enter the Bam file to genotype.See Usage\n"
	);
	&Usage();
	exit;
}
...
if ( !defined $refFile ) {
	$logger->warn(
"$process_name: Reference file was not supplied. The default (/home/shahr2/References/Homo_sapiens_assembly19.fasta) will be used.\n"
	);
	$refFile =
	  "/dmp/data/pubdata/hg-fasta/production/Homo_sapiens_assembly19.fasta";
}
if ( !defined $pathToSamtools ) {
	$logger->warn(
"$process_name: Path for samtools was not supplied. The default (/home/shahr2/Software/Samtools/current/samtools) will be used.\n"
	);
	$pathToSamtools = "/home/shahr2/Software/Samtools/current/samtools";
}
if ( !defined $pathToBedtools ) {
	$logger->warn(
"$process_name: Path for bedtools was not supplied. The default (/home/shahr2/Software/BEDTools/current/bin/) will be used.\n"
	);
	$pathToBedtools = "/home/shahr2/Software/BEDTools/current/bin/";
}
if ( !defined $mbq ) {
	$logger->warn(
		"$process_name: Minimum base quality was not supplied. MBQ will be set to 5.\n");
	$mbq = 5;
}
if ( !defined $mmq ) {
	$logger->warn(
		"$process_name: Minimum mapping quality was not supplied. MMQ will be set to 5.\n");
	$mmq = 5;
}
if ( !defined $outdir ) {
	$logger->warn(
"$process_name: Output directory was not supplied. The current working directory will be used.\n"
	);
	$outdir = getcwd;
}
if ( !defined $outFile ) {
...
#Check  for qsub
if ( !defined $QSUB ) {
	$logger->warn("$process_name: Path to QSUB command not given, Assuming we will run LSF");
}
else {
...
#Check  for bsub
if ( !defined $BSUB ) {
	$logger->warn("$process_name: Path to BSUB command not given, Assuming we will run SGE");
}
else {
...
if ( !defined $queue ) {
	$logger->warn(
		"$process_name: Name of the SGE/LSF queue not given default (all.q) will be used.\n");
	$queue = "all.q";
}
else {
	$logger->info("$process_name: SGE/LSF Queue:$queue\n");
}

#Check  for type of Sample
if ( !defined $typeOfSample ) {
	$logger->warn("$process_name: Type of Sample not given default (Tumor) will be used.\n");
	$typeOfSample = "Tumor";
}
&MakeCSH($outdir);
my ( $bedFile, $mergedBedFile, $vcfHash, $mutationHash ) =
  &MakeBedFile( $filteredMutationVcfFile, $mpileUpOutFile, $outdir );
my ($mpileupOut) =
  &RunMpileup( $bamFile, $mergedBedFile, $mpileUpOutFile, $bamId );
my ( $pileupHash, $pileupstatsHash ) =
  &AnalyzeMpileup( $mpileupOut, $mutationHash );
my ($mergeOutFile) =
  &Merge_Vcf_mpileup( $vcfHash, $mutationHash, $pileupHash, $pileupstatsHash,
	$outdir, $outFile, $bamId );
my @deleteFiles = ();
...
if ( $deleteFiles == 1 ) {
	$logger->info("$process_name: No house keeping performed.\n");
}

#--Calculate and print total runtime (current time minus start time)
$now = time - $now;
$now = sprintf( "%02d:%02d:%02d", int( $now / 3600 ), int( ( $now % 3600 ) / 60) , int( $now % 60 ) );
$logger->info( "$process_name: Total running time: $now\n" );
$logger->info( "$process_name: Process Completed.\n" );
...
sub Usage {
	...
	print "\nUsage : GenotypeAllele.pl [options]
        ...
        [--typeOfSample|tos                S Type of Sample (default:Tumor,optional)]
				[--pname|pn                        S Process name for logging clarity in cases were this process if called as a subproces (default: GTA, optional)]
...
sub MakeCSH
 {
        ...
        if ( -e $filename ) {
            $logger->warn( "$process_name: $filename already exists!");
            return;
        }
        else    {
	...
	if($@){$logger->fatal("$process_name: Cannot change permissions for $filename. Error:$@");exit(1);}
        return;
    }
}
###################################################
...
sub WaitToFinish {
	...
	$logger->info( "$process_name: Waiting for the Process to finish...");
	...
	foreach my $wfile (@waitfilenames) {
		...
		if ($@) { $logger->warn("$process_name: Cannot remove $outdir/$wfile. Error:$@"); }
	}
	return;
}
###################################################
...
sub MakeBedFile {
	...
	open( OUT, ">", "$outdir/$intervalsFile" )
	  || die $logger->fatal("Cannot open $intervalsFile,$!\n");
	while (<IN>) {
		next if ( $_ =~ /^#/ );
		chomp($_);
		my @dataCols = split( "\t", $_ );
		...
		$pileupStart = $start - 1;
		if ( length($ref) > 1 && length($alt) > 1 )    #complex
		{
			my $indel;
			my $indel_len = abs( length($alt) - length($ref) );
			if ( length($ref) > length($alt) ) {
				$pileupEnd = $start + length($ref);
				$indel = "-" . $indel_len . substr( $alt, 1 );
				for my $x ( $start .. $pileupEnd ) {
					$altHash{ $chrom . ":" . $x }
					  { $sampleId . ":complex:" . "complex:" } = $indel;
				}

			}
			if ( length($alt) > length($ref) ) {
				$pileupEnd = $start + length($alt);
				$indel = "+" . $indel_len . substr( $alt, 1 );
				for my $x ( $start .. $pileupEnd ) {
					$altHash{ $chrom . ":" . $x }
					  { $sampleId . ":complex:" . "complex:" } = $indel;
			...
			if ( ( abs( length($ref) ) ) == ( abs( length($alt) ) ) ) {
				$pileupEnd = $start + ( length($alt) - 1 );
				for my $x ( $start .. $pileupEnd ) {
					$altHash{ $chrom . ":" . $x }
					  { $sampleId . ":complex:" . "complex:" } = $alt;
				}
			}

		}
		elsif ( ( abs( length($ref) ) ) == ( abs( length($alt) ) ) ) {
			$pileupEnd = $start;
			$altHash{ $chrom . ":" . $start }
			  { $sampleId . ":" . $alt . ":" . $ref } = $alt;
		}
		else {
			$pileupEnd = $start + 1;
			my $indel;
			...
			if ( length($ref) > length($alt) )    #deletion
			{
				...
				$altHash{ $chrom . ":" . $start }
				  { $sampleId . ":" . $alt . ":" . $ref } = $indel;
				$altHash{ $chrom . ":" . $pileupEnd }
				  { $sampleId . ":" . $alt . ":" . $ref } = $indel;
			}
	...
	if (    ( -e "$outdir/$mergedBedFile" )
		and ( -s "$outdir/$mergedBedFile" != 0 ) )
	{
		$logger->info(
			"$outdir/$mergedBedFile exists andbed file will not be created.\n");
	}
	else {
...
sub MakeProperBed {
	...
	$mergedBedFile =~ s/\.bed/_sorted_merged\.bed/;
	eval {
`$pathToBedtools/sortBed -i $outdir/$bedFile | $pathToBedtools/bedtools merge -d 1 > $outdir/$mergedBedFile`;
	};
	if ($@) {
...
sub RunMpileup {
	...
	my $command =
	  "mpileup -f $refFile -l $bed -C 0 -A -B -Q $mbq -q $mmq -d 50000 $bam";
	if (    ( -e "$outdir/$mpileUpOutFile" )
		and ( -s "$outdir/$mpileUpOutFile" != 0 ) )
	{
		$logger->info(
			"$outdir/$mpileUpOutFile exists and mpileup will not be ran.");
...
"$BSUB -q $queue -J RunPileup.$bamId.$$ -cwd $outdir -e RunPileup.$bamId.$$.%J.stderr -o RunPileup.$bamId.$$.%J.stdout -We 0:59 -R \"rusage[mem=5]\" -M 8 -n 1 \"$pathToSamtools $command\"";
				$logger->debug($cmd);
`$BSUB -q $queue -J RunPileup.$bamId.$$ -cwd $outdir -e RunPileup.$bamId.$$.%J.stderr -o RunPileup.$bamId.$$.%J.stdout -We 0:59 -R "rusage[mem=5]" -M 8 -n 1 "$pathToSamtools $command"`;
`$BSUB -q $queue -cwd $outdir -w "post_done(RunPileup.$bamId.$$)" -J NotifyMpileup.$bamId.$$ -e NotifyMpileup.$bamId.$$.%J.stderr -o NotifyMpileup.$bamId.$$.stat -We 0:59 -R "rusage[mem=2]" -M 4 -n 1 "$outdir/Notify.csh"`;
			}
		};
		if ($@) {
			$logger->fatal("$process_name: unMpileup: Job submission failed. Error: $@");
		}
		push( @notifyNames, "NotifyMpileup.$bamId.$$.stat" );
	}

	&WaitToFinish( $outdir, @notifyNames );
	$logger->info("$process_name: Completed running mpileup on bam file");
	return ("$outdir/$mpileUpOutFile");
}
###################################################
###################################################
...
sub AnalyzeMpileup {
	...
	open( FH, "$mpileupFile" )
	  || die $logger->fatal("Can not open $mpileupFile. Error: $!");
		...
		while ( my ( $id, $value ) = each(%$sampleId) ) {
			...
			if ( $id =~ m/complex/ ) {
				foreach my $key ( sort { $a cmp $b } keys %baseHash ) {
					...
					#print "ComplexKey: $chr,$pos,$key,$id,$value\n";
					$flag = 1;
					my $cnt_f = 0;
					my $cnt_r = 0;

					#print "BaseHash: ", %baseHash, "\n";
					my @posct = ();
					my @negct = ();
					foreach my $mykey ( keys %baseHash ) {
						next if ( $mykey eq uc($ref) or ( $mykey eq "-" ) );

						#print "key: $mykey, value: $baseHash{$mykey}\n\n";
						push @posct, $baseHash{$mykey}{"+"};
						push @negct, $baseHash{$mykey}{"-"};
					}
					$cnt_f = max(@posct);
					$cnt_r = max(@negct);
					my $total = $cnt_f + $cnt_r;

					#print "ADVAL: ","${key}=${total}\t${cnt_f}\t${cnt_r}\n\n";
...
sub Merge_Vcf_mpileup {
	my ( $orgVcfHash, $mutationHash, $mpileupHash, $mpileupstatsHash, $outdir,
		$outFile, $bamId )
	  = @_;
	...
	open( FH, ">", "$outdir/$outFile" )
	  || die $logger->fatal("Cannot open $outdir/$outFile,$!");
	print FH
"Ref_BAM\tSample\tChrom\tPOS\tRef\tAlt\tTotal_Depth\tRef_Depth\tAlt_Counts\tAlt_Freq\tRef_Forward\tRef_Reverse\tAlt_Forward\tAlt_Reverse\n";
	foreach my $key ( keys %vcfHash ) {
		my ( $chr, $pos, $id, $ref, $alt, $qual, $filter, $info ) =
		  split( "\t", $vcfHash{$key} );
		...
		my (@g) = ();
		if ( ( abs( length($ref) ) ) > 1 && ( abs( length($alt) ) ) > 1 ) {
			( $dp, $tf, $tff, $tfr, $ad, $aff, $afr, $vf ) = 0;
			if (
				exists $pileupstatsHash{ $chr . ":" . $pos }
				{ $sampleId . ":complex:" . "complex:" } )
			{
				my $endrange = 0;
				if ( ( abs( length($ref) ) ) > ( abs( length($alt) ) ) ) {
					$endrange = abs($pos) + abs( length($ref) );
				}
				elsif ( ( abs( length($ref) ) ) < ( abs( length($alt) ) ) ) {
					$endrange = abs($pos) + abs( length($alt) );
				}
				else {
					$endrange = abs($pos) + ( abs( length($alt) ) - 1 );
				}
				my @dp  = ();
				my @tf  = ();
				my @tff = ();
				...
				for my $x ( ($pos) .. $endrange ) {
					( $dp, $tf, $tff, $tfr, $ad, $aff, $afr, $vf ) = 0;
					if (
						exists $pileupstatsHash{ $chr . ":" . $x }
						{ $sampleId . ":complex:" . "complex:" } )
					{
						my $tval = $pileupstatsHash{ $chr . ":" . $x }
						  { $sampleId . ":complex:" . "complex:" };

#print "Coord =", $chr, ":", $x, " Sample:", $sampleId, " AltRef: ", $alt, ">", $ref, "\n";
#print " Val = ", $tval, "\n\n";
						($dp) =
						  $pileupstatsHash{ $chr . ":" . $x }
						  { $sampleId . ":complex:" . "complex:" } =~
						  /DP\=(\d+)/;
						@g = split(
							'\; ',
				...
				#print "\tDP:@dp", "\tAD:", "@ad\n\n";
				$dp = max(@dp);
				my $dpIdx = firstidx { $_ == $dp } @dp;
				$tf  = $tf[$dpIdx];
				$tff = $tff[$dpIdx];
				$tfr = $tfr[$dpIdx];
				$ad  = max(@ad);
				my $adIdx = firstidx { $_ == $ad } @ad;
				$aff = $aff[$adIdx];
				$afr = $afr[$adIdx];
				$vf  = 0;
				$vf  = sprintf( "%.5f", ( $ad / $dp ) ) if ( $dp != 0 );
			...
			if (
				...
				{ $sampleId . ":" . $alt . ":" . $ref } )
			{
				($dp) = $pileupstatsHash{ $chr . ":" . $pos }
				  { $sampleId . ":" . $alt . ":" . $ref } =~ /DP\=(\d+)/;
				@g = split(
					'\; ',
					$pileupstatsHash{ $chr . ":" . $pos }
					  { $sampleId . ":" . $alt . ":" . $ref }
				);
				( $tf, $tff, $tfr ) = split( ",", $g[1] );
				my @h = split( '\=', $g[2] );
			...
			my $pos1 = $pos + 1;
			if ( !exists $pileupstatsHash{ $chr . ":" . $pos1 }
				{ $sampleId . ":" . $alt . ":" . $ref } )
			{
				if ( !( $typeOfSample =~ m/NTC/i ) ) {
					$logger->warn(
"$bamId:Position ahead for $key did not report pileup stats"
					);
				}
				( $dp, $tf, $tff, $tfr, $ad, $aff, $afr, $vf ) = 0;
			}
				...
				($dp) = $pileupstatsHash{ $chr . ":" . $pos1 }
				  { $sampleId . ":" . $alt . ":" . $ref } =~ /DP\=(\d+)/;
				@g = split(
					'\; ',
					$pileupstatsHash{ $chr . ":" . $pos1 }
					  { $sampleId . ":" . $alt . ":" . $ref }
				);
				( $tf, $tff, $tfr ) = split( ",", $g[1] );
				@g = split(
				...
				if ( length($ref) < length($alt) )
				{ # fix added 20140108 by AZ. For insertions, RefDepth is equal to TotalDepth. However, it should be TotalDepth-AltDepth.
					$tf = $tf - $ad;
				}
		...
		print FH
"$bamId\t$sampleId\t$chr\t$pos\t$ref\t$alt\t$dp\t$tf\t$ad\t$vf\t$tff\t$tfr\t$aff\t$afr\n";
...
sub HouseKeeping {
	my ( $outdir, @list ) = @_;
	$logger->info("$process_name: Removing Unwanted files...");
	foreach my $file (@list) {
		if ( $file =~ /\// ) {
			eval { `rm $file`; };
			if ($@) {
				$logger->fatal("$process_name: Can not remove $file,Error:$@");
			}
		}
		else {
			eval { `rm $outdir/$file`; };
			if ($@) {
				$logger->fatal("$process_name: Can not remove $outdir/$file,Error:$@");
			}
		}
	}
	return;
